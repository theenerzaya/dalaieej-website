"use client";

import { useLocale } from "next-intl";
import { testimonials, type LocalizedText } from "@/app/data/testimonials";
import FadeInBlock from "@/app/components/getting-here/FadeInBlock";

const SCALLOP_PATH =
  "M 2360.460938 805.90625 C 2420.511719 907.925781 2425.410156 1038.398438 2361.933594 1148.1875 C 2268.015625 1310.632812 2060.003906 1366.308594 1897.363281 1272.515625 C 1648.910156 1126.527344 1327.367188 1167.003906 1119.304688 1364.425781 L 1119.160156 1364.425781 C 914.996094 1170.699219 600.402344 1124.53125 343.714844 1271.011719 C 241.589844 1330.984375 110.925781 1335.859375 1 1272.484375 C -161.640625 1178.667969 -217.367188 970.914062 -123.484375 808.476562 C 22.695312 560.394531 -17.855469 239.238281 -215.523438 31.445312 L -215.523438 31.300781 C -21.53125 -172.613281 24.671875 -486.773438 -121.984375 -743.160156 C -182.046875 -845.160156 -186.933594 -975.65625 -123.445312 -1085.429688 C -29.535156 -1247.90625 178.476562 -1303.589844 341.113281 -1209.796875 C 589.5625 -1063.835938 911.105469 -1104.316406 1119.179688 -1301.738281 L 1119.324219 -1301.738281 C 1329.683594 -1102.128906 1646.074219 -1065.289062 1897.386719 -1209.800781 C 2060.023438 -1303.59375 2268.035156 -1247.929688 2361.957031 -1085.476562 C 2425.433594 -975.679688 2420.53125 -845.164062 2360.460938 -743.144531 C 2213.824219 -486.757812 2260.054688 -172.636719 2454.023438 31.28125 L 2454.023438 31.421875 C 2260.027344 235.355469 2213.824219 549.515625 2360.460938 805.90625 Z";

function localize(text: LocalizedText | undefined, locale: string) {
  if (!text) return undefined;
  return locale === "mn" ? text.mn : text.en;
}

function Scallop({ position }: { position: "top" | "bottom" }) {
  return (
    <svg
      aria-hidden="true"
      viewBox="0 1100 2238.75 270"
      preserveAspectRatio="none"
      className={[
        "block h-[58px] w-full text-ink sm:h-[68px] md:h-[82px]",
        position === "top"
          ? "-mb-px rotate-180 drop-shadow-[0_-10px_18px_rgba(13,15,28,0.1)]"
          : "-mt-px drop-shadow-[0_10px_18px_rgba(13,15,28,0.14)]",
      ].join(" ")}
    >
      <path fill="currentColor" d={SCALLOP_PATH} />
    </svg>
  );
}

type ContextualTestimonialProps = {
  testimonialId: string;
  eyebrow?: LocalizedText;
  className?: string;
};

export default function ContextualTestimonial({
  testimonialId,
  eyebrow,
  className,
}: ContextualTestimonialProps) {
  const locale = useLocale();
  const isMn = locale === "mn";
  const testimonial = testimonials.find((item) => item.id === testimonialId);

  if (!testimonial) return null;

  const quote = localize(testimonial.quote, locale);
  const metaParts = [
    localize(testimonial.location, locale),
    localize(testimonial.source, locale),
    localize(testimonial.date, locale),
  ].filter(Boolean);

  return (
    <FadeInBlock className={className} delay={0.08} distance={18}>
      <aside aria-label={isMn ? "Зочны сэтгэгдэл" : "Guest reflection"}>
        <Scallop position="top" />
        <figure className="bg-ink px-5 py-12 text-center shadow-2xl ring-1 ring-white/10 sm:px-8 md:px-12 md:py-16">
          {eyebrow ? (
            <p className="mb-8 font-cta text-[10px] uppercase tracking-[0.28em] text-main/50 sm:text-xs">
              {localize(eyebrow, locale)}
            </p>
          ) : null}
          <span aria-hidden className="mx-auto block h-px w-12 bg-main/20" />
          <blockquote
            className={[
              "mx-auto mt-8 max-w-2xl text-pretty text-center text-main/85",
              isMn ? "font-editorial-mn" : "font-editorial-en",
              "italic font-light",
              "text-2xl sm:text-3xl md:text-[2.15rem]",
              "leading-[1.45] md:leading-[1.5]",
            ].join(" ")}
          >
            &ldquo;{quote}&rdquo;
          </blockquote>
          <figcaption className="mt-8 flex flex-col items-center gap-2">
            <span className="font-cta text-xs uppercase tracking-[0.3em] text-main/70">
              &mdash;&nbsp;{testimonial.author}
            </span>
            {metaParts.length > 0 ? (
              <span className="font-body text-xs text-main/45">
                {metaParts.join(" · ")}
              </span>
            ) : null}
          </figcaption>
        </figure>
        <Scallop position="bottom" />
      </aside>
    </FadeInBlock>
  );
}
